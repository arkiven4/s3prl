...
def get_preprocess_args():
    ...
    parser.add_argument('--n_jobs', default=8, type=int, help='Number of jobs used for feature extraction', required=False)
...
def acoustic_preprocess(args, tr_set, dim):
    for s in tr_set:
        df = pd.read_csv(args.data_path + "/metadata_ssl.csv." + s)
        print('')
        print('Preprocessing', s, 'data...', end='')
        todo = list(df['path_file'].values)
        todo_label = list(df['label'].values)
        ...
        tr_x = Parallel(n_jobs=args.n_jobs)(delayed(extract_feature)(str(file), feature=args.feature_type, \
                                            ...
                                            save_feature=os.path.join(cur_path, str(file).split('/')[-1].replace('.wav', ''))) for file in tqdm(todo))

        # sort by len
        sorted_todo = [os.path.join(s, str(todo[idx]).split('/')[-1].replace('.wav', '.npy')) for idx in reversed(np.argsort(tr_x))]
        sorted_label = [str(todo_label[idx]) for idx in reversed(np.argsort(tr_x))]
        # Dump data
        df = pd.DataFrame(data={'file_path': [fp for fp in sorted_todo], 'length':list(reversed(sorted(tr_x))), 'label': [fp for fp in sorted_label]})
